...
- Задача похожа на задачу, которую я решал в 2022 году: https://github.com/PolushinM/Hierarchical-Classifier.
...
### В полученном для работы присутствуют следующие свойства объектов (EDA.ipynb):
...
- **title** - заголовок, самое значимое для классификации поле. 
...
**shop_title** - наименование магазина \
...
Данные делились на train/test в соотношении 80/20 со стратификацией по category_id, либо на 5 фолдов на кросс-валидации, для обучения перед сабмитом - в соотношении 97,5/2,5 (тестовая выборка использовалась для определения лучшего чекпоинта).

Одна из особенностей - большой дисбаланс: есть множество категорий, в которые попадают всего 1-2 товара, есть категории с тысячами товаров. 
...
**Пример, когда модель предсказывает хорошо, но категория все равно не совпадает с разметкой (таких примеров много):** \
...
**Пример, когда модель предсказывает неплохо, но немного не попадает в категорию (таких примеров абсолютное большинство):** \
...
Улучшать скор необходимо за счет 2й и 3й категории ошибок, но из 3й категории много выжать невозможно. Улучшить результаты на второй категории ошибок, скорее всего, можно за счёт увеличения размера выборки: даже лёгкая модель rubert_tiny2 начинает быстро переобучаться, скорее всего, она сильно улучшит точность, если добавить данных.

### Сводная таблица результатов.
| №   | Model | Валидация | F1 |
| --- | ----------- | ----------- |:----:|
| 1. |	Fasttext | Кросс-валидация, 5 фолдов | 0.8672 |
| 2. |	LaBSE | Тестовая выборка 20% | 0.8879 |
| 3. |	rubert_tiny2 | Тестовая выборка 20% |  0.8849 |
| 4. |	CLIP (Изображения) | Тестовая выборка 20% | 0.64 |
| 5. |	CLIP + BERT (rubert_tiny2) | Тестовая выборка 20% | **0.8902** | 
| 6. |	CLIP (Изображения) | Тестовая выборка 2,5%, лучший чекпоинт | 0.725 |
| 7. |	CLIP + BERT (rubert_tiny2) | Тестовая выборка 2,5%, лучший чекпоинт | **0.902** | 

На этот раз лучший скор показал BERT (в прошлый раз был FastText).  
Выводы такие: 
- хотя FastText показывает высокий скор на подобных текстах (описания товаров больше похожи на набор слов, чем на осмысленный текст), BERT лучше, если уметь правильно его готовить.
- BERT имеет ещё одно важное преимущество: масштабируемость. В маркетплейсах размещаются миллионы товаров (иногда - миллиарды). FastText ограничен своим алгоритмом усреднения всех ембеддингов токенов, и не может выявлять сложные зависимости, что вполне работает, если выборка небольшая. Трансформеры могут выявлять очень сложные зависимости в данных, если выборка большая, чем больше данных, тем более "умную" модель можно обучить, не ясно даже, где предел (можно взглянуть на современные LLM). Это значит, можно ожидать, что по мере увеличения объема выборки, точность модели будет по-немногу увеличиваться и не упрётся в "потолок".
- изображения неожиданно мало "докинули" точности. Вероятно, текстовое описание и так даёт исчерпывающую информацию, не нуждающуюся в иллюстрациях фотографиями. Потенциал есть: по мере увеличения объема выборки и сложности мультимодальной модели можно добиваться улучшения качества классификации, это оправдано, если другие возможности исчерпаны.

### Submit (submit.ipynb)
Для итогового расчета использовалась модель CLIP + BERT (clip-vit-large-patch14 + rubert_tiny2), где CLIP был предварительно обучен, сгенерированы его эмбеддинги. BERT обучался вместе с мета-моделью, содержащей 2 механизма внимания. \
Сначала было выполнено обучение с 20% тестовым сетом, затем, обучение с теми же гиперпараметрами на 97,5% датасета с выбором лучшего чекпоинта по оставшимся 2,5%. Эмбеддинги CLIP (CLIP-Pictures_only-dim64) с лучшего чекпоинта оказались несколько переобученными, поэтому пришлось сделать некоторую (очень костыльную, но времени уже не было) регуляризацию, в результате получено F1=0.902 (в один момент случайно получилось F1=0.914, когда я остановил обучение в ноутбуке и запустил вновь, но этот результат воспроизвести не удалось, к сожалению).
